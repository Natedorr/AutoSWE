...
Installed plugins can also bundle lifecycle config through their plugin manifest or a default`hooks/hooks.json`file. SeeBuild plugins[Build
plugins](/codex/plugins/build#bundled-mcp-servers-and-lifecycle-config)for the plugin packaging rules.
...
````[[hooks.PreToolUse]]matcher ="^Bash$"[[hooks.PreToolUse.hooks]]type ="command"command ='/usr/bin/python3 "$(git rev-parse --show-toplevel)/.codex/hooks/pre_tool_use_policy.py"'timeout =30statusMessage ="Checking Bash command"[[hooks.PostToolUse]]matcher ="^Bash$"[[hooks.PostToolUse.hooks]]type ="command"command ='/usr/bin/python3 "$(git rev-parse --show-toplevel)/.codex/hooks/post_tool_use_review.py"'timeout =30statusMessage ="Reviewing Bash output"````
...
If you need the full wire format, seeSchemas[Schemas](#schemas).
...
Fields in addition toCommon input fields[Common input fields](#common-input-fields):
...
JSON on`stdout`supportsCommon output fields[Common output fields](#common-output-fields)and this hook-specific shape:
...
JSON on`stdout`supportsCommon output fields[Common output fields](#common-output-fields). If a matching`PreCompact`hook returns`continue: false`, Codex stops before compacting.
...
JSON on`stdout`supportsCommon output fields[Common output fields](#common-output-fields). If a matching`PostCompact`hook returns`continue: false`, Codex stops after compacting.
...
JSON on`stdout`supportsCommon output fields[Common output fields](#common-output-fields). To ask Codex to continue the subagent flow, return:
...
JSON on`stdout`supportsCommon output fields[Common output fields](#common-output-fields). To keep Codex going, return:
...
If you need the exact current wire format, see the generated schemas in theCodex GitHub repository[Codex GitHub repository](https://github.com/openai/codex/tree/main/codex-rs/hooks/schema/generated).