...
Upgrade your app to the latest OpenAI API models.Difficulty**Intermediate**Time horizon**1h**
...
# Contents
← All use cases[← All use cases](/codex/use-cases)Copy pageExport as PDF[Export as PDF](/codex/use-cases/api-integration-migrations/?export=pdf)

Use Codex to update your existing OpenAI API integration to the latest recommended models and API features, while checking for regressions before you ship.Intermediate1h

Related linksLatest model guide[Latest model guide](/api/docs/guides/latest-model)Prompt guidance[Prompt guidance](/api/docs/guides/prompt-guidance)OpenAI Docs MCP[OpenAI Docs MCP](/learn/docs-mcp)Evals guide[Evals guide](/api/docs/guides/evals)
...
All the specifics about the new API features and model behavior are documented in our docs, in thelatest model[latest model](/api/docs/guides/latest-model)andprompt guidance[prompt guidance](/api/docs/guides/prompt-guidance)guides.

The OpenAI Docs skill also includesspecific guidance[specific guidance](https://github.com/openai/codex/blob/6323f0104d17d211029faab149231ba787f7da37/codex-rs/skills/src/assets/samples/openai-docs/references/upgrading-to-gpt-5p4.md)as a concrete migration reference. For the current upgrade target, use thelatest model[latest model](/api/docs/guides/latest-model)guide.
...
Thiscookbook guide[cookbook guide](/cookbook/examples/evaluation/building_resilient_prompts_using_an_evaluation_flywheel)covers in detail how to do this using ourEvals API[Evals API](/api/docs/guides/evals).
