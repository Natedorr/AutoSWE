...
Turn your use cases into focused apps for ChatGPT.Difficulty**Advanced**Time horizon**1h**
...
# Contents
← All use cases[← All use cases](/codex/use-cases)Copy pageExport as PDF[Export as PDF](/codex/use-cases/chatgpt-apps/?export=pdf)

Build one narrow ChatGPT app outcome end to end: define the tools, scaffold the MCP server and optional widget, connect it in ChatGPT, and iterate until the core flow works.Advanced1h

Related linksApps SDK quickstart[Apps SDK quickstart](/apps-sdk/quickstart)Build an MCP server[Build an MCP server](/apps-sdk/build/mcp-server)Testing[Testing](/apps-sdk/deploy/testing)
...
- ChatGPT Apps[ChatGPT Apps](https://github.com/openai/skills/tree/main/skills/.curated/chatgpt-apps)Plan tools, wire MCP resources, and follow the current ChatGPT app build flow.
- OpenAI Docs[OpenAI Docs](https://github.com/openai/skills/tree/main/skills/.curated/openai-docs)Pull current official Apps SDK guidance before Codex writes code or suggests architecture.
- Vercel[Vercel](https://github.com/openai/plugins/tree/main/plugins/vercel)Bring Vercel ecosystem guidance into Codex with curated skills and the official Vercel MCP server.
...
- Iterate on metadata, state handling,`structuredContent`, and`_meta`payloads until the core read flow behaves reliably inside ChatGPT.
...
- Prepare a hosted preview with a stable`/mcp`endpoint, verify streaming and widget asset hosting, and review the launch checklist before sharing or submitting the app.
...
**Plan the App Before You Scaffold It**Use $chatgpt-apps with $openai-docs to plan a ChatGPT app for [use case] in this repo. Requirements: - Start with one core user outcome. - Propose 3-5 tools with clear names, descriptions, inputs, and outputs. - Recommend whether v1 needs a widget or can start data-only. - Prefer TypeScript for the MCP server and React for the widget. - Call out auth, deployment, and test requirements. Output: - Tool plan - Proposed file tree - Golden prompt set - Risks and open questions

**Scaffold the First Working Version**Use $chatgpt-apps with $openai-docs to scaffold the first version of this ChatGPT app. Stack: - TypeScript MCP server - React widget - Vite build - Local HTTPS via ngrok Constraints: - Keep the app narrow: one read flow and at most one write flow. - Return concise structuredContent for the model and reserve widget-only data for _meta. - Make tool handlers idempotent. - Reuse existing repo patterns before adding dependencies. Verification: - Start the local server - Explain how to connect the app in ChatGPT developer mode - List the exact prompts to test

**Add Auth Only After the Core Flow Works**Use $chatgpt-apps with $openai-docs to add auth to this ChatGPT app. Requirements: - Keep read-only tools anonymous if possible. - Add OAuth 2.1 only for user-specific data or write actions. - Use an existing identity provider such as Auth0 or Stytch. - Document scopes, token checks, and the developer-mode test flow. Output: - Auth flow summary - Server changes - Required environment variables - End-to-end test plan

**Prepare the App for Deployment and Review**Use $chatgpt-apps with $openai-docs and @vercel to prepare this ChatGPT app for a hosted preview. Requirements: - Expose a stable HTTPS /mcp endpoint. - Keep streaming responses working on /mcp. - Host widget assets correctly. - Add a launch-readiness checklist covering metadata, tool hints, privacy, and test prompts. Output: - Deployment plan - Preview URL or hosting steps - Review checklist - Remaining risks
...
Widget framework | React[React](https://react.dev/) | A strong default for stateful widgets, especially when the UI needs filters, tables, or multi-step interaction.
Hosting | Vercel[Vercel](https://vercel.com/docs) | Quick deploys, preview environments, automatic HTTPS, and a clear path to hosted MCP endpoints.
