...
Use Codex as a scored improvement loop to solve hard tasks.Difficulty**Advanced**Time horizon**Long-running**
...
# Contents
← All use cases[← All use cases](/codex/use-cases)Copy pageExport as PDF[Export as PDF](/codex/use-cases/iterate-on-difficult-problems/?export=pdf)

Give Codex an evaluation system, such as scripts and reviewable artifacts, so it can keep improving a hard task until the scores are good enough.AdvancedLong-running

Related linksCustom instructions with AGENTS.md[Custom instructions with AGENTS.md](/codex/guides/agents-md)Codex workflows[Codex workflows](/codex/workflows)
...
- Set a target for the overall score.
- Set a separate target for the LLM-judge average.
- Tell Codex to continue until both are above the threshold, not just one.
...
- Run the evals on the current baseline.
- Identify the biggest failure mode from the scores and artifacts.
- Make one focused change that addresses that bottleneck.
- Re-run the evals.
- Log the new scores and whether the change helped.
- Continue until the thresholds are met.
...
Use Codex to map unfamiliar codebases, explain different modules and data flow, and point...EngineeringAnalysis[Understand large codebasesUse Codex to map unfamiliar codebases, explain different modules and data flow, and point...EngineeringAnalysis](/codex/use-cases/codebase-onboarding)
