...
Turn screenshots and visual references into responsive UI with visual checks.Difficulty**Intermediate**Time horizon**1h**
...
# Contents
← All use cases[← All use cases](/codex/use-cases)Copy pageExport as PDF[Export as PDF](/codex/use-cases/frontend-designs/?export=pdf)

Use Codex to translate screenshots and design briefs into code that matches the repo's design system, then use Playwright to compare the implementation to your references for different screen sizes and iterate until it looks right.Intermediate1h

Related linksCodex skills[Codex skills](/codex/skills)
...
The more specific you are about the expected interaction patterns and the style you want, the better the result will be. The model tends to default to high-frequency patterns and style so if it’s not obvious from your references that you want something else, the UI might look generic. The more input you give, be it more reference inspiration or more specific instructions, the more you can expect to have a UI that stands out.
...
Make sure you have the Playwright interactive skill enabled in Codex. For more details, see theskills documentation[skills documentation](/codex/skills).
...
### Suggested follow-up prompt[current implementation image] [reference image] This doesn't look right. Make sure to implement something that matches closely the reference: [if needed, specify what is different]
...
Use Codex to pull design context, assets, and variants from Figma, translate them into code...Front-endDesign[Turn Figma designs into codeUse Codex to pull design context, assets, and variants from Figma, translate them into code...Front-endDesign](/codex/use-cases/figma-designs-to-code)
