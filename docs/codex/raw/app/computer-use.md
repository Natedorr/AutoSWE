...
For web apps you are building locally, use thein-app browser[in-app browser](/codex/app/browser)first.
...
````Open the app with computer use, reproduce the onboarding bug, and fix thesmallest code path that causes it. After each change, run the same UI flowagain.````
...
When you enable locked computer use, Codex installs an Appleauthorization plug-in[authorization plug-in](https://developer.apple.com/documentation/security/authorization-plug-ins)that participates in the macOS unlock flow.
