...
- If`approvals_reviewer = "auto_review"`, Codex routes that approval request to a separate reviewer agent instead of stopping for a person.
...
The exact policy lives in the open-source Codex repository:policy_template.md[policy_template.md](https://github.com/openai/codex/blob/main/codex-rs/core/src/guardian/policy_template.md)andpolicy.md[policy.md](https://github.com/openai/codex/blob/main/codex-rs/core/src/guardian/policy.md). That policy can be customized per enterprise with`guardian_policy_config`or per user with local`[auto_review].policy`[[auto_review].policy](/codex/config-advanced#approval-policies-and-sandbox-modes).
...
For setup details, seeManaged configuration[Managed configuration](/codex/enterprise/managed-configuration#configure-automatic-review-policy).

The default reviewer policy is in the open-source Codex repository:core/src/guardian/policy.md[core/src/guardian/policy.md](https://github.com/openai/codex/blob/main/codex-rs/core/src/guardian/policy.md). Enterprises can replace its tenant-specific section with`guardian_policy_config`in managed requirements. Individual users can also set a local`[auto_review].policy`[[auto_review].policy](/codex/config-advanced#approval-policies-and-sandbox-modes)in their`config.toml`, but managed requirements take precedence:
...
- Add narrow`writable_roots`[writable_roots](/codex/config-advanced#approval-policies-and-sandbox-modes)for scratch directories or neighboring repos you intentionally use.
- Add narrowly scopedprefix rules[prefix rules](/codex/rules). Prefer precise command prefixes such as`["cargo", "test"]`or`["pnpm", "run", "lint"]`over broad patterns such as`["python"]`or`["curl"]`. Broad rules often erase the very boundary Auto-review is meant to guard.
...
For the research rationale and published evaluation results, see theAlignment Research post on Auto-review[Alignment Research post on Auto-review](https://alignment.openai.com/auto-review/).