...
This page explains the core concepts and tradeoffs. For setup, agent configuration, and examples, seeSubagents[Subagents](/codex/subagents).
...
For background, see the Chroma writeup oncontext rot[context rot](https://research.trychroma.com/context-rot).
...
Higher reasoning effort increases response time and token usage, but it can improve quality for complex work. For details, seeModels[Models](/codex/models),Config basics[Config basics](/codex/config-basic), andConfiguration Reference[Configuration Reference](/codex/config-reference).