...
|---|---|
| `add` | Set or change a scalar field value. **Note:** on `System.Tags` `add` is additive — it merges into the existing tag set. To replace tags, use a single `replace` (see [replace-tags-on-work-item.md](replace-tags-on-work-item.md)). || `remove` | Clear a field (no `value` needed) |
