...
| `TEST_GATE` | `true` | Post-fix test gate (Natedorr/testProject#20): after the fix agent commits/pushes, `coder._finalize_fix` runs the repo's test suite in the worktree before the task can reach terminal `fixed`. A red suite lands the task in the non-terminal `test_failed` status (a comment carries the failure, `/pr` is blocked, `/fix`/`/retry` restart with a fresh attempt budget; with `AUTO_FIX_ON_GATE_FAILURE` on, `/fix` also auto-dispatches — see below). Skips (no resolvable command, missing runner, timeout, no tests collected) are non-gating. Set `false` to disable. |
...
| `LINK_COMMIT_TRAILER` | `true` | Append the provider's `commit_trailer()` (edge E2, issue #245) to the `commit_and_push` commit message — GitHub `"Refs #N"`, Azure `"#N"` — so the commit shows up in the issue's Development/links timeline. Set `false` to opt out. |
| `AUTO_CLOSE_ON_MERGE` | `true` | Whether `ensure_links` issues an explicit `IssueTracker.close_issue` write when a merge is observed and the platform will not close the issue itself (Azure always; GitHub only if the closing keyword failed to register). Set `false` to opt out of the explicit close entirely. |
| `done_state` | `""` (discover, else `"Closed"`) | Azure only (issue #245 §1.5): the `System.State` value `close_issue` writes. Resolution order: per-repo `done_state` in `repos.json` → this global value → runtime discovery (`workitemtypes/{type}/states`, the `Completed`-category state) → fallback `"Closed"` (reported via a one-time comment on a rejecting 400). Deliberately lowercase to mirror the `repos.json` key casing. |
| `CI_WATCH` | `true` | Issue #245 plan §2.2/§2.3: enables the poll-time CI read (`providers.adapter.read_ci`) for tasks resting in `CI_WATCH_STATUSES` (`fixed`, `shipped`, `synced`, `ci_failed`) with a resolvable branch. Surfaced via `queue status` and the `/sync` comment, and consumed by `decide()`: a red build either auto-dispatches a fix (`AUTO_FIX_ON_GATE_FAILURE`) or parks the task at `ci_failed` with a comment, green clears it back (or re-fires a deferred `create_pr`, §2.6), an unreachable API gets a one-time warning. Set `false` to disable the watch entirely for a repo (per-repo override: lowercase `ci_watch`). |
| `CI_POLL_INTERVAL_SEC` | `120` | Throttle on `read_ci`: at most one CI API call per watched task per this many seconds, checked against the persisted `ci_last_checked` watermark — keeps a `--drain` loop from hammering the API. Per-repo override: lowercase `ci_poll_interval_sec`. |
| `AUTO_FIX_ON_GATE_FAILURE` | `true` | Issue #245 plan §2.3-§2.5: the shared recoverable-gate switch — on a red build (`ci_failed`) **or** a red local test-gate run (`test_failed`), `decide()` auto-dispatches a `/fix` (`Action(kind="fix", trigger="ci"|"gate")`) with the failure text as guidance, instead of only parking the task. Bounded by the four brakes documented in [safeguards.md](safeguards.md) — a shared attempt counter (`GATE_MAX_FIX_ATTEMPTS`), a per-commit watermark, a reset-on-green/human-command rule, and this gate itself. Set `false` to downgrade a repo to the old detect-and-park behaviour for **both** signals (recovery via a human `/fix`) — the escape hatch for a flaky or permanently red pipeline/suite. Per-repo override: lowercase `auto_fix_on_gate_failure`. (Renamed from `CI_AUTO_FIX`, which only covered the CI signal.) |
| `GATE_MAX_FIX_ATTEMPTS` | `2` | The shared gate-triggered fix budget — a counter (`gate_attempt_count`) kept separate from the phase `attempt_count` so a gate auto-fix loop (CI or local test gate) can never consume the budget a human `/fix` depends on. Resets to 0 on a green signal or any human-dispatched Claude action, never on a push the agent itself made. Per-repo override: lowercase `gate_max_fix_attempts`. (Renamed from `CI_MAX_FIX_ATTEMPTS`.) |
| `CI_LOG_MAX_CHARS` | `4000` | Per-failure excerpt budget passed to `VCSProvider.get_ci_failures(..., max_chars=...)` — the provider truncates so the text handed to the fix prompt is already bounded. |

**`done_states`** (per-repo only, in `repos.json`; default `["Closed", "Done", "Removed"]`) is the terminal-state set the *read* side (`list_open_issues`, `_to_normalized`) treats as closed. `load_repos_config(cfg)` validates at config load that an Azure entry's effective `done_state` is a member of its effective `done_states` — a mismatch is a startup `ValueError`, since writing a non-terminal state would leave the work item rediscovered on every poll.

**Integer keys re-parsed:** After loading the file, `AGENT_TIMEOUT`, `AGENT_RETRY_ON_FAILURE`, `MAX_ATTEMPTS`, `MAX_TOTAL_HOURS`, `MAX_CONCURRENT`, `MAX_DRAIN_CYCLES`, `TEST_GATE_TIMEOUT`, `MAX_TURNS`, `REVIEW_MAX_TURNS`, `CI_POLL_INTERVAL_SEC`, `GATE_MAX_FIX_ATTEMPTS`, and `CI_LOG_MAX_CHARS` are cast to `int` (a malformed value falls back to its default).

**Boolean keys re-parsed:** `SILENT_REPORTING`, `MINIMAL_POSTING`, `AUTO_ASSIGN`, `AUTO_CREATE_PR`, `LINK_BRANCH_TO_ISSUE`, `PR_REQUIRE_SYNC`, `PR_REQUIRE_CI`, `AUTO_PURGE_BRANCHES`, `TEST_GATE`, `LINK_COMMIT_TRAILER`, `AUTO_CLOSE_ON_MERGE`, `CI_WATCH`, and `AUTO_FIX_ON_GATE_FAILURE` are compared to `"true"` (case-insensitive) after file load.
