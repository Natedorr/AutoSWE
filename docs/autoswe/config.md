...
| `CI_WATCH` | `true` | Issue #245 plan §2.2/§2.3: enables the poll-time CI read (`providers.adapter.read_ci`) for tasks resting in `CI_WATCH_STATUSES` (`fixed`, `shipped`, `synced`, `ci_failed`) with a resolvable branch. Surfaced via `queue status` and the `/sync` comment, and consumed by `decide()`: a red build either auto-dispatches a fix (`CI_AUTO_FIX`) or parks the task at `ci_failed` with a comment, green clears it back, an unreachable API gets a one-time warning. Set `false` to disable the watch entirely for a repo (per-repo override: lowercase `ci_watch`). |
...
| `CI_AUTO_FIX` | `true` | Issue #245 plan §2.3/§2.4, P4: on a red build, `decide()` auto-dispatches a `/fix` (`Action(kind="fix", trigger="ci")`) with the CI failure text as guidance, instead of only parking the task at `ci_failed`. Bounded by the four brakes documented in [safeguards.md](safeguards.md) — a separate attempt counter (`CI_MAX_FIX_ATTEMPTS`), a per-commit SHA watermark, a reset-on-green/human-command rule, and this gate itself. Set `false` to downgrade a repo to the old detect-and-park behaviour (recovery via a human `/fix`) — the escape hatch for a flaky or permanently red pipeline. Per-repo override: lowercase `ci_auto_fix`. |
| `CI_MAX_FIX_ATTEMPTS` | `2` | The CI-triggered fix budget — a counter (`ci_attempt_count`) kept separate from the phase `attempt_count` so a CI auto-fix loop can never consume the budget a human `/fix` depends on. Resets to 0 on a green build or any human-dispatched Claude action, never on a push the agent itself made. Per-repo override: lowercase `ci_max_fix_attempts`. |
...
**Integer keys re-parsed:** After loading the file, `AGENT_TIMEOUT`, `AGENT_RETRY_ON_FAILURE`, `MAX_ATTEMPTS`, `MAX_TOTAL_HOURS`, `MAX_CONCURRENT`, `MAX_DRAIN_CYCLES`, `TEST_GATE_TIMEOUT`, `MAX_TURNS`, `REVIEW_MAX_TURNS`, `CI_POLL_INTERVAL_SEC`, `CI_MAX_FIX_ATTEMPTS`, and `CI_LOG_MAX_CHARS` are cast to `int` (a malformed value falls back to its default).

**Boolean keys re-parsed:** `SILENT_REPORTING`, `MINIMAL_POSTING`, `AUTO_ASSIGN`, `AUTO_CREATE_PR`, `LINK_BRANCH_TO_ISSUE`, `PR_REQUIRE_SYNC`, `PR_REQUIRE_CI`, `AUTO_PURGE_BRANCHES`, `TEST_GATE`, `LINK_COMMIT_TRAILER`, `AUTO_CLOSE_ON_MERGE`, `CI_WATCH`, and `CI_AUTO_FIX` are compared to `"true"` (case-insensitive) after file load.
