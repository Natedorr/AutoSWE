...
| `CI_WATCH` | `true` | Issue #245 plan §2.2/§2.3: enables the poll-time CI read (`providers.adapter.read_ci`) for tasks resting in `CI_WATCH_STATUSES` (`fixed`, `shipped`, `synced`, `ci_failed`) with a resolvable branch. Surfaced via `queue status` and the `/sync` comment, and (since P3) consumed by `decide()`: a red build parks the task at `ci_failed` with a comment, green clears it back, an unreachable API gets a one-time warning. No auto-dispatch yet — that's `CI_AUTO_FIX`, a later phase. Set `false` to disable the watch for a repo (per-repo override: lowercase `ci_watch`). |
...
**Integer keys re-parsed:** After loading the file, `AGENT_TIMEOUT`, `AGENT_RETRY_ON_FAILURE`, `MAX_ATTEMPTS`, `MAX_TOTAL_HOURS`, `MAX_CONCURRENT`, `MAX_DRAIN_CYCLES`, `TEST_GATE_TIMEOUT`, `MAX_TURNS`, `REVIEW_MAX_TURNS`, and `CI_POLL_INTERVAL_SEC` are cast to `int` (a malformed value falls back to its default).

**Boolean keys re-parsed:** `SILENT_REPORTING`, `MINIMAL_POSTING`, `AUTO_ASSIGN`, `AUTO_CREATE_PR`, `LINK_BRANCH_TO_ISSUE`, `PR_REQUIRE_SYNC`, `PR_REQUIRE_CI`, `AUTO_PURGE_BRANCHES`, `TEST_GATE`, `LINK_COMMIT_TRAILER`, `AUTO_CLOSE_ON_MERGE`, and `CI_WATCH` are compared to `"true"` (case-insensitive) after file load.
