...
| `CI_WATCH` | `true` | Issue #245 plan §2.2 (P2, report-only): enables the poll-time CI read (`providers.adapter.read_ci`) for tasks resting in `CI_WATCH_STATUSES` (`fixed`, `shipped`, `synced`, `ci_failed`) with a resolvable branch. `decide()` does not act on the result yet — it's surfaced via `queue status` and the `/sync` comment only. Set `false` to disable the watch for a repo (per-repo override: lowercase `ci_watch`). |
