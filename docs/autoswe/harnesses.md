...
**No `mode` (issue #166).** Codex does NOT advertise the `mode` capability: it accepts `RunSpec.mode` for contract parity but performs no read-only enforcement (no `--sandbox` mapping, no per-tool gating). A plan or review phase that resolves to a Codex profile therefore has no sandbox, no tool allowlist, and no Bash file-mutation guard. Handlers detect this via `has_read_only_enforcement` (no `mode` AND no `can_use_tool`) and **loudly degrade** — a prominent warning log — then rely on the post-run `ensure_worktree_unchanged` backstop (see [Sandbox / bypass policy](#codex-phase-4)) to roll back any edits the agent made. The same gap also disables the WAITING flow on Codex: AskUserQuestion interception lives entirely in the `can_use_tool` callback, so a Codex plan session can never ask the user a question.

**Capabilities (not yet supported):** `mode` (no read-only enforcement — see [No `mode`](#codex-phase-4) above), `mcp` (no MCP comment posting), `can_use_tool` (no per-tool gating), `plan_permission` (no dedicated plan mode), `session_fork` (no fork primitive), `structured_output` (Codex has no JSON-Schema-validated structured output, so `RunSpec.output_format` is ignored and the planner/reviewer fall back to their text-pattern paths). Handlers degrade gracefully when these are unavailable — e.g. the planner falls back to text parsing instead of MCP plan posting, and a plan/review phase without read-only enforcement loudly degrades and rolls back edits post-run (issue #166).
...
- **An `isError` tool result does NOT flip the run to `error`.** A tool-result event with `isError: true` is a *non-terminal* signal: pi catches the tool failure, reports it to the LLM, and continues (`docs/pi/extensions.md` — "the thrown error is caught, reported to the LLM with `isError: true`, and execution continues"). The agent normally recovers, so a clean rc-0 run that emitted a valid final `message_end` is `"success"` even if a mid-run tool had a nonzero exit — a transient `grep`-matched-nothing or a `pytest` the agent then fixed does not discard the run's real, verified work. The failure is logged for the operator but does not set the run's error flag. Whether the fix actually landed is decided downstream (the `committed` flag from `commit_and_push` + the post-fix test gate), not by a mid-run tool exit code. Genuine process-level failures — `extension_error` events, `error` events, and stream overflow — still set the flag and sink the run to `error`. (Revised: the earlier "any isError flips to error" behavior mislabeled recovered runs as failed, surfacing as a live E2E where a completed docstring fix with a passing test suite was reported `subtype=error` and never committed.)
