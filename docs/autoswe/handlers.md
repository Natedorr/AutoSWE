...
- **CI-triggered dispatch (issue #245 plan §2.3/§2.4, P4):** `decide()`'s CI watch can dispatch `/fix` itself — `Action(kind="fix", trigger="ci")` — on a red build, with no slash command or `triggering_comment_id` involved. `run()` routes it through the same `_run_fix_with_sync()` path as a human `/fix`, first appending real failure text (`VCSProvider.get_ci_failures()`, fetched lazily here — not in the read cycle) to the `guidance` `decide()` built from the `CIStatus` summary. `emit()` recognises `trigger == "ci"` and bumps the separate `ci_attempt_count` / `ci_last_fixed_sha` watermark instead of the phase `attempt_count`, and preserves (rather than nulls) `last_dispatched_command_id` / `last_consumed_reply_id` since there is no triggering comment. See `safeguards.md` for the four-brake budget.
