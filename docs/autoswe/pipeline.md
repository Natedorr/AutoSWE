...
2. **`gh_closed` detection** — Issues that dropped out of the open-issues list get `gh_closed = True`. If a real dispatch happened, `autoswe_status` becomes that phase's COMPLETED status (e.g. `"fixed"` after a `/fix`); if the task was never dispatched, the status stays neutral (`None`) and no terminal label is written — a fabricated `autoswe:fixed` on a fresh, never-worked issue is a false terminal state that would make drivers skip the fix (issue #258). A per-poll invariant heal resets any never-dispatched entry carrying a COMPLETED status back to `None` and actively clears the stale `autoswe:*` label off the live issue (`tracker.clear_status`, no-op when no such label exists) (it runs only while the issue is open — a poisoned entry whose issue is still closed keeps `autoswe_status="fixed"` until the issue is reopened or `queue prune` removes it). Reopened issues clear the flag.
