...
| `ci_attempt_count` | `int` | CI auto-fix budget counter (issue #245 plan §2.4, P4) — deliberately **separate** from `attempt_count` so a CI loop can never spend the budget a human `/fix` depends on. Compared against `CI_MAX_FIX_ATTEMPTS`; resets to 0 on a green build (`ci_recovered`) or any human-dispatched Claude action, never on a push the agent itself made. |
| `ci_last_fixed_sha` | `str \| None` | SHA watermark (brake 2): the head commit an auto-fix has already been dispatched for. `decide()` refuses to dispatch a second auto-fix for the same commit even if the counter were somehow lost, bounding the loop structurally rather than only by policy. Cleared on a green build. |
