...
| `attempt_count` | `int` | Dispatch attempts; `decide()` carries `task.attempt_count + 1` forward on every restart (terminal / review-blocking / `planned`, slash or plain-text) and `/retry` is the only path that resets it to 1. When the stored value already equals `MAX_ATTEMPTS`, the next restart trips the `MAX_ATTEMPTS` guard (`mark_failed_limit`, `limit_reason="attempts"`) |
