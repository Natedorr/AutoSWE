...
| `gate_attempt_count` | `int` | Shared recoverable-gate auto-fix budget counter (issue #245 plan §2.5) — one counter drives auto-fix recovery for **both** `test_failed` and `ci_failed`, deliberately **separate** from `attempt_count` so a gate loop can never spend the budget a human `/fix` depends on. Compared against `GATE_MAX_FIX_ATTEMPTS`; resets to 0 on a green signal (`ci_recovered`) or any human-dispatched Claude action, never on a push the agent itself made. (Renamed from `ci_attempt_count`, which was CI-only.) |
| `gate_last_fixed_sha` | `str \| None` | Per-commit watermark (brake 2), shared by both gate signals: the head/local commit an auto-fix has already been dispatched for. `decide()` refuses to dispatch a second auto-fix for the same commit even if the counter were somehow lost, bounding the loop structurally rather than only by policy. Cleared on a green build. (Renamed from `ci_last_fixed_sha`.) |
| `test_failed_sha` | `str \| None` | The commit the branch suite is currently red for — set by `emit()` when the post-fix test gate lands the task at `test_failed`. Compared against `gate_last_fixed_sha` by `decide()`'s local test-gate auto-fix branch so the same commit is never auto-fixed twice. |
| `test_failure_detail` | `str \| None` | The pytest output tail (`harness/test_gate.py`'s `GateResult.reason` + `.output`) captured when the task landed in `test_failed`, reused verbatim by the shared prompt-assembly helper (`orch/gate_policy.build_gate_guidance`) to build the next auto-fix's guidance without re-running anything. |
| `test_gate_limit_notified` | `bool` | One-time flag: once the shared gate budget is exhausted for a `test_failed` task, this suppresses repeating the "budget exhausted" comment every poll (mirrors `ci_error_notified`). Cleared whenever `gate_attempt_count` resets. |
| `pr_deferred` | `bool` | Set by the `create_pr` effect (issue #245 plan §2.6) when preflight defers PR creation on pending/failing/unreachable CI, instead of telling the user to re-post `/pr`. The next green CI observation makes `decide()` emit `Action(kind="retry_deferred_pr")`, which re-fires `create_pr` and clears this flag; `find_existing_pr`'s idempotency guard makes the re-emit safe even if a PR was created in the meantime. |
