...
| `session_id` | `str` | Claude SDK session ID; used to resume planning |
...
| `_comment_id` | `int` | Progress comment ID for the current in-flight dispatch |
...
Provider-agnostic. Cached at the test seam between Layer A (decide) and Layer B (run) / Layer C (emit).
...
### `DispatchResult` — Claude handler output (Layer B output)
...
Returned by `run()` for Claude actions. Pure actions (skip, abort, noop) return `None`.
