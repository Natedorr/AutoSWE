...
**pi** enforces read-only via the tool **allowlist** (`--tools`), unlike Codex: `mode` is translated into a real `--tools` set at the CLI level, so a `plan`/`read_only` phase is restricted to the read-only tools (`read,grep,find,ls`) and cannot write files or run `bash`. This is why pi advertises the `mode` capability — plan/review keep their read-only guarantee instead of degrading and relying on the post-run `ensure_worktree_unchanged` rollback backstop Codex needs. The `read_write` mode (used by `/fix`) grants `bash` in the allowlist (`read,bash,edit,write,grep,find,ls`), which is full access — by design on the dedicated isolated machine, consistent with the isolation boundary above. `ask_question` is excluded from every run (it would block a non-interactive run indefinitely).
...
## Recoverable-Gate Auto-Fix Budget — Four Independent Brakes (issue #245 plan §2.4-§2.5)

`AUTO_FIX_ON_GATE_FAILURE` (default on) lets `decide()` respond to a red **gate** — the remote CI watch (`ci_failed`) or the local post-fix test gate (`test_failed`) — by auto-dispatching a `/fix` (`Action(kind="fix", trigger="ci")` or `trigger="gate")`) instead of only parking the task. One policy, two signals (`RECOVERABLE_GATE_STATUSES` in `tracking/labels.py`): both share the same four brakes, each independently sufficient to stop a loop that keeps pushing and re-failing:

1. **`GATE_MAX_FIX_ATTEMPTS` (default 2) — a separate, shared counter.** `gate_attempt_count` is tracked apart from the phase `attempt_count`, so a gate-triggered loop (CI or local) can never spend the budget a human `/fix` depends on (and vice versa: a human retry loop never exhausts the gate budget). Checked in `decide()`'s `_decide_ci` and `_decide_test_gate`; once `gate_attempt_count >= GATE_MAX_FIX_ATTEMPTS`, the next failure produces `Action(kind="mark_failed_limit", limit_reason="ci" | "gate")` instead of another auto-dispatch — `emit()` parks the task (at `ci_failed`, or leaves it at `test_failed`) with a comment asking for a human `/fix` (not `/retry`, which would just re-arm the same loop; `_guard_blocked` stays `False` so `/fix` isn't refused).
2. **Per-commit watermark — the structural brake.** `gate_last_fixed_sha` records the head/local commit an auto-fix has already been dispatched for. `decide()` checks it **before** the counter (`ci.head_sha` for the CI signal, `task.test_failed_sha` for the local one), so even a lost or corrupted counter can't cause a second auto-dispatch for the exact same commit.
3. **Reset rule.** `gate_attempt_count` resets to 0 only on a green signal (the `ci_recovered` action) or any human-dispatched Claude action (`emit()`'s common queue patch resets it whenever `action.trigger not in ("ci", "gate")`) — never on a new commit the agent's own auto-fix pushed. This is what lets a genuinely different regression get a fresh budget while a stuck loop on the same underlying failure still exhausts it.
4. **`AUTO_FIX_ON_GATE_FAILURE` config gate.** Per-repo overridable (lowercase `auto_fix_on_gate_failure` in `repos.json`) escape hatch: `false` downgrades both signals to the old detect-and-park behaviour (comment + `ci_failed`/`test_failed`, recovery via a human `/fix`) for a repo whose pipeline or suite is flaky or permanently red. Not a rollout stage — a durable per-repo setting.
...
Failure text for the fix prompt is assembled by one shared helper, `orch/gate_policy.build_gate_guidance` — the CI signal's text comes from `VCSProvider.get_ci_failures()`, fetched lazily in Layer B (`orch/run.py`) only when a CI-triggered fix is actually dispatched (so a red build that's merely being throttled or already parked never costs a log/annotation download) and truncated provider-side to `CI_LOG_MAX_CHARS` (default 4000); the local test-gate signal's text is the pytest output tail already captured in `task.test_failure_detail` when the task landed at `test_failed` — no extra I/O needed.
