...
``close_issue``) gated by declared ``Capability`` membership, so no caller
knows whether it is talking to GitHub or Azure — the absence of a capability
decides the work, not a provider name.

Edge ownership by phase:
  - ``"branch"``            E1 issue->branch. Only this call site (worktree
    creation) establishes it — the GitHub ``createLinkedBranch`` mutation
    needs the base commit SHA and must run before the branch is pushed.
  - ``"pr_open"``           E3/E4 PR->issue. Writes ``link_pr_to_issue`` when
    the PR is not yet linked (no-op on GitHub, where the closing keyword in
    the body is the link).
  - ``"merge_observation"`` E5 merge->issue-closed. Self-heals E3, and issues
    the explicit ``close_issue`` write when the platform will not auto-close
    (or the closing keyword failed to register).
"""
...
def ensure_links(
    ...
    *task* is the queue entry (a mutable dict carrying at least
    ...
    ``linkage_state`` / ``linkage_missing`` for every phase except
    ``"branch"`` (which runs at worktree creation on a throwaway dict before
    the queue entry exists). A ``None`` task (or one with no ``issue_number``)
    yields ``None`` — nothing to operate on.

    *vcs*, when given, is the already-resolved VCSProvider the caller holds —
    passing it in avoids constructing a redundant client and keeps tests that
    patch that instance effective. When omitted it is resolved from
    *repo_cfg*.

    *base_sha* is the base commit SHA the ``"branch"`` phase needs to issue
    the GitHub ``createLinkedBranch`` mutation (which must run before the
    branch is pushed).
    ...
    if phase == "branch":
        missing: list[str] = []
        _establish_branch(vcs, caps, base_sha, issue_num, branch, missing)
        return LinkageState(missing=tuple(missing))

    # Read first — the steady-state path is a read plus no writes.
    ...
    if phase == "merge_observation" and pr_number is not None:
        _maybe_close_on_merge(cfg, state, repo_cfg, issue_num, missing)
...
def render_linkage_checklist(task: dict) -> str:
    ...
    task with no linkage observation yet renders nothing. Provider-agnostic:
    it reads the normalized state, not a provider field.
    """
    ...
    if state.get("pr_number") is not None:
        lines.append(
            f"  PR #{state['pr_number']}: merged={state.get('merged', False)} "
            f"merge_state={state.get('merge_state', 'unknown')}"
        )
    return "\n".join(lines)


def _establish_branch(vcs, caps, base_sha, issue_num, branch, missing: list[str]) -> None:
    """E1: establish the issue->branch link at worktree creation (best-effort)."""
    if Capability.BRANCH_LINK not in caps:
        # Declared absence — report it, never a silent skip.
...
def _establish_pr_link(vcs, caps, state, issue_num, pr_number, missing: list[str]) -> None:
    """E3/E4: establish the machine-readable PR->issue edge if it is missing."""
    if pr_number is None:
        return  # no PR yet — get_linkage already listed "pr_link"
...
def _maybe_close_on_merge(cfg, state, repo_cfg, issue_num, missing: list[str]) -> None:
    """E5: explicitly close the issue when the platform will not do it itself.

    Idempotent: the current issue state is checked first, so repeated
    merge-observations do not re-close.
    """
    if not state.merged:
        return
    ...
    owed = (Capability.AUTO_CLOSE_ON_MERGE not in tracker.capabilities()) or (not state.closes_on_merge)
    if not owed:
        return
    try:
